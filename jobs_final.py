...
st.set_page_config(page_title='Job Title Predictor', layout='wide', initial_sidebar_state='auto')
...
def main():
    ...
    if not model_choices:
        st.error("No .keras models found in the 'models' folder.")
        return
    ...
    if st.button('Predict Job Title'):
        if not description or len(description.strip()) < 10:
            ...
            csv_buf.seek(0)
            st.download_button(label='Download Results (CSV)', data=csv_buf, file_name='job_predictions.csv', mime='text/csv')
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('---')
    st.markdown('<div class="small">Note: Predictions are based on a trained model and may not be perfect.</div>', unsafe_allow_html=True)
